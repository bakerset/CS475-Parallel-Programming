...
__global__
void MonteCarlo( IN float* dbeforey, IN float* daftery, IN float* ddistx, OUT int* dsuccesses)
{
	...
	// Check if the golf ball lands in the hole
    if (fabs(x - distx) <= RADIUS) {
        dsuccesses[gid] = 1;  // Mark the trial as successful
	}
}
