...
int main( int argc, char *argv[ ] )
{
    ...
    // Print longitude and latitude of each capital city
    if( NUMT == 1 )
    {
        for( int k = 0; k < NUMCAPITALS; k++ )
        {
            ...
            // Uncomment the following line if you did the extra credit to identify closest cities
            // int closestCityIndex = ...; // Index of closest city to Capitals[k]
            // fprintf( stderr, "\t%3d:  %8.2f , %8.2f , %s\n", k, Capitals[k].longitude, Capitals[k].latitude, Cities[closestCityIndex].name.c_str() );
        }
    }
