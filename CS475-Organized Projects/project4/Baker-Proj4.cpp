...
int main( int argc, char *argv[ ] )
{
    ...
    #pragma omp critical
    {
        ...
        if (fileSize == 0) {
            fprintf(outFile, "ARRAYSIZE,SSETime,SSEPerformance,CPPTime,CPPPerformance\n");
        }
    }
	...
	// Perform Non-SIMD multiplication - cpp
    ...
    // Perform SIMD multiplication - asm
	maxPerformance = 0.;
	for( int t = 0; t < NUMTRIES; t++ )
	{
		double time0 = omp_get_wtime( );
	...
	fprintf(outFile, "%d,%f,%f,%f,%f\n", ARRAYSIZE, megaMults, speedup, megaMults, speedup);
